...
Detecting collisions among many moving blobs in 2D gets expensive fast:
checking every pair each frame is $O(n^2)$. A [quad-tree][quadtree] brings
that down by only comparing blobs that share a region of space.
...
query touches roughly $O(\log n)$ cells, so a frame costs about
$O(n \log n)$ instead of $O(n^2)$, and rebuilding the tree each tick keeps
